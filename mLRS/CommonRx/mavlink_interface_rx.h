...
class MavlinkBase
{
  ...
  private:
    ...
    typedef enum {
        ...
        TXBUF_STATE_BURST,      // Buffer filling, pause parameter download
        TXBUF_STATE_OVERFLOW,   // Buffer in danger of overflow, tell parameter download to pause and slow down streams
        TXBUF_STATE_WAIT,       // Buffer was filling, waiting to drain, keep parameter download paused, no change to streams
        TXBUF_STATE_RECOVERING, // Buffer draining, resume parameter download
        TXBUF_STATE_RECOVERED,  // Buffer low for more than 1 second, wait another second then resume normal state
    } TXBUF_STATE_ENUM;
    uint8_t txbuf_state;
...
void MavlinkBase::Do(void)
{
    ...
    // Switch to burst mode when buffer starts to fill.
    // Switch to normal mode when buffer does not reach half full for more than 1 second.
    if (Setup.Rx.SendRadioStatus && connected()) {
        if ((tnow_ms - radio_status_tlast_ms) >= 1000) {
            if (txbuf_state != TXBUF_STATE_NORMAL) {
                // Some time has passed since we entered BURST state without reaching NORMAL;  Check buffer again.
                if (serial.bytes_available() > RX_SERIAL_RXBUFSIZE/2) {
                    // Should not happen unless normal stream rate is too high.
                    // Buffer is not draining properly, report full buffer and hope to reach RECOVERING next time.
                    txbuf_state = TXBUF_STATE_OVERFLOW;
                    // This is a case where BURST and RECOVERING won't balance so we will impact normal stream rates.
                    // This helps us avoid buffer overflow at the beginning and after the GCS changes stream rates.
                    // Note, if we want to slow down normal streams even faster, we could set txbuf_state = TXBUF_STATE_NORMAL
                    // here, which would likely cause txbuf = 0 to be sent twice immediately.
                } else if (txbuf_state == TXBUF_STATE_RECOVERING ) {
                    // Give some extra time to see if we might still be bursting and reach BURST again
                    txbuf_state = TXBUF_STATE_RECOVERED;
                } else if (txbuf_state == TXBUF_STATE_BURST) {
                    // Give some extra time to see if we might reach RECOVERING
                    txbuf_state = TXBUF_STATE_WAIT;
                } else {
                    // Probably will not happen unless normal stream rates are too high.
                    // We haven't reached seen RECOVERING or BURST for 2 seconds. Switch to normal to slow down streams
                    txbuf_state = TXBUF_STATE_NORMAL;
                }
            }
            radio_status_tlast_ms = tnow_ms;
            inject_radio_status = true;
        } else if ((txbuf_state != TXBUF_STATE_OVERFLOW) && (serial.bytes_available() > RX_SERIAL_RXBUFSIZE*7/8)) {
            buzzer.BeepLP();
            // Should not happen.  Burst did not stop as expected
            txbuf_state = TXBUF_STATE_OVERFLOW;
            radio_status_tlast_ms = tnow_ms;
            inject_radio_status = true;
        } else if ((txbuf_state != TXBUF_STATE_BURST) && (txbuf_state != TXBUF_STATE_OVERFLOW) && (serial.bytes_available() > RX_SERIAL_RXBUFSIZE/2)) {
            txbuf_state = TXBUF_STATE_BURST;
            radio_status_tlast_ms = tnow_ms;
            inject_radio_status = true;
        } else if ((txbuf_state >= TXBUF_STATE_BURST) && (txbuf_state <= TXBUF_STATE_WAIT) && (serial.bytes_available() < FRAME_RX_PAYLOAD_LEN*2)) {
            txbuf_state = TXBUF_STATE_RECOVERING;
            radio_status_tlast_ms = tnow_ms;
            inject_radio_status = true;
...
uint8_t MavlinkBase::_calc_txbuf(void)
{
    ...
    if (Setup.Rx.RadioStatusMethod == RADIO_STATUS_METHOD_W_TXBUF) {
        ...
        else if (txbuf_state == TXBUF_STATE_OVERFLOW) txbuf = 0; // 33 didn't stop the flow as expected; May be normal stream rate is too high
        else if (txbuf_state == TXBUF_STATE_RECOVERING) {
            txbuf = 93; // restart data flow for Ardupilot or PX4
        } else if (txbuf_state == TXBUF_STATE_WAIT) {
            txbuf = 50; // maintain normal stream rates for Ardupilot or PX4
        } else if (txbuf_state == TXBUF_STATE_RECOVERED) {
            txbuf = 51; // continue data flow for Ardupilot
            //txbuf_state = TXBUF_STATE_NORMAL;
        }
        ...
        if (txbuf_state == TXBUF_STATE_NORMAL) {
            ...
            txbuf = 33; // no choice but to slow down normal stream rates for PX4
        } else if (txbuf_state == TXBUF_STATE_RECOVERED) {
            txbuf = 50; // continue data flow for PX4
            //txbuf_state = TXBUF_STATE_NORMAL;
        }
...
else if(txbuf_state == TXBUF_STATE_WAIT) dbg.puts("wait , ");
else if(txbuf_state == TXBUF_STATE_OVERFLOW) dbg.puts("over, ");
else if(txbuf_state == TXBUF_STATE_RECOVERING) dbg.puts("resu , ");
else if(txbuf_state == TXBUF_STATE_RECOVERED) dbg.puts("reco, ");
else dbg.puts("norm, ");
...
if(txbuf>90) dbg.puts("-20 "); else dbg.puts("0 ");
        if (txbuf_state == TXBUF_STATE_RECOVERED) {
            txbuf_state = TXBUF_STATE_NORMAL;
        }
#endif
